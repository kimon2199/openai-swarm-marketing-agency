...
def extract_response_info(response):
    ...
    if hasattr(response, "tool_calls") and response.tool_calls:
        return {"tool_calls": response.tool_calls[0].function.name}
    return {"message": response.messages[-1]["content"]}

def run_function_evals(agent, test_cases, n=1, eval_path=None):
    """Run function evaluations for the marketing agency agents."""
    correct_function = 0
    results = []
    ...
    for test_case in test_cases:
        ...
        print(f"\033[92mCorrect functions for this case: {case_correct} out of {n}\033[0m")
